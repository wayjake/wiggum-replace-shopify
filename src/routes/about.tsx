// 🏫 About Page - The story behind Enrollsy
// Why we exist and what we believe about school technology
...
export const Route = createFileRoute('/about')({
  head: () => ({
    meta: [
      { title: 'About Us | Enrollsy' },
      {
        name: 'description',
        content: 'Enrollsy was built by people who believe private schools deserve better software. Learn about our mission to modernize school enrollment.',
      },
      { property: 'og:title', content: 'About Us | Enrollsy' },
      { property: 'og:description', content: 'Modernizing school enrollment, one school at a time.' },
    ],
  }),
  component: AboutPage,
});
      ...
      <div className="max-w-6xl mx-auto px-6 py-4">
        <nav className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <Link to="/about" className="text-[#1F2A44] font-medium">About</Link>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden pt-4 pb-2 border-t border-gray-100 mt-4">
            <div className="flex flex-col gap-4">
              ...
              <Link to="/contact" className="bg-[#2F5D50] text-white px-5 py-2 rounded-md text-center font-medium">
                Request Demo
              </Link>
            </div>
          </div>
  ...
  return (
    <section className="py-20 px-6 bg-white">
      <div className="max-w-3xl mx-auto text-center">
        <h1 className="text-4xl md:text-5xl font-display text-[#1F2A44] mb-6 leading-tight">
          Schools deserve software that{' '}
          <span className="text-[#2F5D50]">actually works</span>
        </h1>
        <p className="text-lg text-[#5F6368] leading-relaxed">
          ...
          Modern, intuitive, and built specifically for how private schools actually operate.
        </p>
      </div>
    </section>
  );
    ...
    <section className="py-20 px-6 bg-[#F7F5F2]">
      <div className="max-w-5xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-sm font-medium text-[#2F5D50] mb-4">OUR MISSION</p>
            <h2 className="text-3xl md:text-4xl font-display text-[#1F2A44] mb-6">
              From first inquiry to tuition paid—without the chaos
            </h2>
            ...
            <p className="text-[#5F6368]">
              Enrollsy brings admissions, enrollment, and billing into one clean system
              that school staff actually enjoy using—and parents love.
            </p>
          </div>

          <div className="bg-white p-8 rounded-lg border border-gray-200">
            <h3 className="font-display text-xl text-[#1F2A44] mb-4">What we believe</h3>
        ...
        <div className="grid sm:grid-cols-2 gap-8">
          {values.map((value, index) => (
            <div key={index} className="p-6 border border-gray-200 rounded-lg">
              <div className="w-10 h-10 bg-[#F7F5F2] rounded-lg flex items-center justify-center mb-4">
                <value.icon className="w-5 h-5 text-[#2F5D50]" />
              </div>
              <h3 className="text-lg font-display text-[#1F2A44] mb-2">{value.title}</h3>
    ...
    <section className="py-20 px-6 bg-[#1F2A44] text-white">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="prose prose-lg text-white/80 max-w-none">
          ...
          <p className="mb-6">
            We started Enrollsy because we believed there had to be a better way. A system that's
  ...
  return (
    <section className="py-20 px-6 bg-[#F7F5F2]">
      <div className="max-w-2xl mx-auto text-center">
        <h2 className="text-3xl font-display text-[#1F2A44] mb-4">
          Ready to see it in action?
        </h2>
        <p className="text-[#5F6368] mb-8">
          Schedule a demo and we'll show you how Enrollsy can transform your enrollment process.
        </p>
